...
def verify_tables_exist(engine):
    ...
    with engine.connect() as conn:
        result = conn.execute(text(
            ...
            "('taxons','observations','audit_events','convex_hulls','grid_metadata','base_grid_cells')"
        ))
        existing_tables = {row[0] for row in result}

        required = {'taxons', 'observations', 'audit_events', 'convex_hulls', 'grid_metadata', 'base_grid_cells'}
